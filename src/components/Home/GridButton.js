...
export default function GridButton({isPlayed = false, soundPlay, id, handleChangeSample}) {
    return <Wrapper isPlayed={isPlayed}
                    onClick={soundPlay}>
        <label onClick={(e)=> e.stopPropagation()} htmlFor={id}>🎵</label>
        <input onClick={(e)=> e.stopPropagation()} id={id} type="file" onChange={handleChangeSample} />
    </Wrapper>
}
